        ...
        if (data.lat && data.lng) {
          if (data.threatScore > 0) {
            fetchedThreats.push({ 
              ...
              severity: data.threatScore || 1, 
              locationName: data.locationText || "Incident Zone" 
            });
          }
        }
      });
        ...
        {activeView === "map" ? (
          <>
            <div className="flex-1 relative">
              <MapComponent showHeatmap={isHeatmapActive} />
            </div>
            <div className="z-[1000] relative bg-white border-t border-gray-200">
              <ResourceForm />
            </div>
          </>
        ) : (
          <div className="flex-1 overflow-auto bg-slate-50">
            <CitizenVolunteerPortal />
          </div>
        )}
      </div>
    </main>
